...
def build_ai_system_prompt(mode, context, reply_length="short"):
    """Build sport-specific, mode-aware system prompt with tone and safety rules"""
    ...
    # Tone instructions
    if tone == "strict":
        tone_instruction = "Be direct, concise, and no-nonsense. Push the athlete to improve."
    else:
        tone_instruction = "Be encouraging, supportive, and clear. Build confidence while guiding."
    
    # Length instruction
    length_instruction = "Keep responses brief (2-4 sentences)." if reply_length == "short" else "Provide detailed responses (4-8 sentences)."
    
    # Base system prompt
    base_prompt = f"""You are an expert {sport} coach for {name} (level: {level}).
{tone_instruction}
{length_instruction}

CRITICAL RULES:
1. ONLY provide advice specific to {sport}. If asked about other sports, politely redirect: "I specialize in {sport}. Let me help you with that instead."
2. Every response MUST include WHAT to do AND WHY it matters (1-2 lines for why).
3. NO medical diagnosis or treatment. For injuries, only suggest rest, consult professionals, or general recovery tips.
4. NO guarantees about results. Use language like "typically helps" or "often improves".
5. Be specific to the user's data when available.

SAFETY DISCLAIMER (include when relevant): "This is general guidance. Consult a professional for personalized advice."
"""
    ...
    if mode == "training":
        ...
        if training:
            mode_context = f"""
TRAINING CONTEXT:
- Training {training.get('days', 'N/A')} days/week, {training.get('minutes', 'N/A')} minutes/session
- Current fatigue level: {training.get('fatigue', 'N/A')}
- Plan started: {training.get('plan_start_date', 'N/A')}

Focus on: workout structure, drills, periodization, recovery balance for {sport}."""
        else:
            mode_context = f"\nFocus on: training advice, drills, and workout planning for {sport}."
        ...
        if diet:
            mode_context = f"""
DIET CONTEXT:
- Diet type: {diet.get('diet_type', 'N/A')}
- Budget: {diet.get('budget', 'N/A')}
- Allergies/restrictions: {diet.get('allergies', 'None specified')}

Focus on: nutrition timing, meal suggestions, hydration for {sport} athletes."""
        else:
            mode_context = f"\nFocus on: sports nutrition and meal planning for {sport} athletes."
...
- Affected area: {injury.get('body_part', 'N/A')}
- Pain level: {injury.get('pain', 'N/A')}/10
- Recovery stage: {injury.get('stage', 'N/A')}

Focus on: safe modifications, recovery exercises, when to rest. ALWAYS recommend consulting a physiotherapist for serious concerns."""
        else:
            mode_context = f"\nFocus on: injury prevention and recovery guidance for {sport}. Always recommend professional consultation."
        ...
        if tournament:
            mode_context = f"""
TOURNAMENT CONTEXT:
- Upcoming event: {tournament.get('upcoming', 'N/A')}
- Days until event: {tournament.get('days_left', 'N/A')}
- Category: {tournament.get('category', 'N/A')}
- Importance: {tournament.get('importance', 'N/A')}

Focus on: competition prep, peaking strategy, mental preparation, taper advice for {sport}."""
        else:
            mode_context = f"\nFocus on: tournament preparation and competition strategy for {sport}."
    
    else:
        # General/Ask mode - include all context
        mode_context = f"\nProvide general {sport} coaching. Use any relevant context from training, diet, injury, or tournament data."
    
    return base_prompt + mode_context

def generate_fallback_response(mode, context):
    """Generate deterministic fallback when OpenAI is unavailable"""
    ...
    # Tone prefix
    if tone == "strict":
        prefix = f"{name}, here's what you need to know:"
    else:
        prefix = f"Great question, {name}! Here's my advice:"
    
    fallback_responses = {
        "training": f"""{prefix}

WHAT: Focus on consistency over intensity. Aim for quality practice sessions with proper warm-up and cool-down specific to {sport}.

WHY: Consistent training builds muscle memory and prevents injury, which is crucial for long-term improvement in {sport}.

Note: AI coach is currently resting. For personalized plans, try again later or consult a local {sport} coach.""",

        "diet": f"""{prefix}

WHAT: Prioritize balanced meals with lean protein, complex carbs, and vegetables. Stay hydrated before, during, and after {sport} sessions.

WHY: Proper nutrition fuels performance and recovery. {sport.title()} athletes need sustained energy and quick recovery.

Note: AI coach is currently resting. For specific meal plans, try again later or consult a sports nutritionist.""",

        "injury": f"""{prefix}

WHAT: If experiencing pain, follow RICE (Rest, Ice, Compression, Elevation). Avoid pushing through sharp or persistent pain.

WHY: Early intervention prevents minor issues from becoming serious injuries that could sideline you from {sport}.

IMPORTANT: This is general guidance only. Please consult a physiotherapist or sports medicine professional for proper diagnosis.

Note: AI coach is currently resting. Try again later for more specific advice.""",

        "tournament": f"""{prefix}

WHAT: In the days before competition, reduce training intensity (taper), focus on sleep, visualize your performance, and review your {sport} strategy.

WHY: Tapering allows your body to recover fully while mental preparation builds confidence for peak performance.

Note: AI coach is currently resting. For detailed competition strategy, try again later."""
    }
    
    return fallback_responses.get(mode, f"""{prefix}

WHAT: Stay consistent with your {sport} practice, listen to your body, and focus on gradual improvement.

WHY: Long-term progress comes from sustainable habits, not quick fixes.

Note: AI coach is currently resting. Please try again later for personalized advice.""")
...
def forgot_password_post():
    ...
    - Render forgot_password.html with success message
    ...
    # Always render with generic success message
...
def save_tournament():
    """Proper error handling for tournament save"""
    uid = session.get("user_id")
    if not uid:
        return jsonify(success=False, error="Please login again."), 401
...
# ======================================================
# COACH API (CORE AI LITE)
# ======================================================
@app.route("/api/coach", methods=["POST"])
def coach():
    """
    Core AI (Lite) Coach API
    - Supports modes: training, diet, injury, tournament, Ask (general)
    - Sport-specific responses based on user's active_sport
    - Tone-aware (calm/strict) from users.coach_tone
    - Memory-lite: reads from training/diet/injury/tournament tables
    - WHY-EXPLAIN: every response includes WHAT + WHY
    - Safety limits: no medical diagnosis, no guarantees
    """
    uid = session.get("user_id")
    if not uid:
    ...
    try:
        ...
        reply_length = settings["reply_length"] if settings else "short"
        ...
        if mismatched_sport:
            ...
            reply = f"I noticed you're asking about {mismatched_sport}, but you're training for {user_sport}. I specialize in {user_sport} coaching.\n\nWHAT: Let me help you with {user_sport}-specific advice instead.\n\nWHY: Sport-specific guidance is more effective for your goals. What would you like to know about {user_sport}?"
        ...
        if client is None:
            reply = generate_fallback_response(mode, context)
        else:
            try:
                system_prompt = build_ai_system_prompt(mode, context, reply_length)
                ...
                res = client.chat.completions.create(
                    ...
                    max_tokens=300 if reply_length == "short" else 600,
                    temperature=0.7
                )

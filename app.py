...
def init_db():
    ...
    # ======================================================
    # NEXIVO UNIFIED SCORING ENGINE TABLES
    # ======================================================
    # IMPROVEMENT 1: Updated default baseline values
    cur.execute("""
    CREATE TABLE IF NOT EXISTS athlete_metrics (
        user_id INTEGER PRIMARY KEY,
        recovery_score INTEGER DEFAULT 75,
        confidence_score INTEGER DEFAULT 50,
        injury_risk INTEGER DEFAULT 10,
        performance_score INTEGER DEFAULT 60,
        burnout_risk TEXT DEFAULT 'low',
        tournament_readiness INTEGER DEFAULT 50,
...
# ======================================================
# NEXIVO UNIFIED SCORING ENGINE HELPERS
# ======================================================
def initialize_metrics_if_missing(user_id):
    """Initialize athlete_metrics row if not exists"""
    try:
        conn = get_db()
        existing = conn.execute("SELECT 1 FROM athlete_metrics WHERE user_id=?", (user_id,)).fetchone()
        if not existing:
            # IMPROVEMENT 1: Updated default baseline values
            conn.execute("""
                INSERT INTO athlete_metrics
                (user_id, recovery_score, confidence_score, injury_risk, performance_score,
                 burnout_risk, tournament_readiness, consistency_score, last_calculated_at)
                VALUES (?, 75, 50, 10, 60, 'low', 50, 50, ?)
...
def log_event(user_id, event_type, reference=None):
    """Log an event to the event_log table"""
    try:
        conn = get_db()
        conn.execute(
            "INSERT INTO event_log (user_id, event_type, reference, created_at) VALUES (?, ?, ?, ?)",
            (user_id, event_type, reference, datetime.now().isoformat()))
        conn.commit()
        conn.close()
    except Exception:
        pass
...
def recalculate_metrics(user_id):
    """Recalculate all athlete metrics based on current data using rule-based integer math"""
    try:
        conn = get_db()

        # Fetch training data
        training = conn.execute(
            "SELECT days, minutes, fatigue FROM training WHERE user_id=?",
            (user_id,)
        ).fetchone()

        # Fetch injury data
        injury = conn.execute(
            "SELECT status, pain, stage FROM injury WHERE user_id=?",
            (user_id,)
        ).fetchone()

        # Fetch tournament data
        tournament = conn.execute(
            "SELECT upcoming, days_left, importance FROM tournament WHERE user_id=?",
            (user_id,)
        ).fetchone()
        ...
        burnout_risk = "low"
        tournament_readiness = 50
        consistency_score = 50

        # ======================================================
        # CONSISTENCY SCORE (based on training existence)
        # ======================================================
        if training:
            consistency_score = 70
            days = training["days"] or 0
            minutes = training["minutes"] or 0
            if days >= 4:
        ...
        # ======================================================
        # RECOVERY SCORE (based on fatigue and injury)
        # ======================================================
        if training:
            fatigue = (training["fatigue"] or "").lower()
            if fatigue == "low":
                recovery_score = 85
                ...
                recovery_score = 40
            elif fatigue == "very high":
                recovery_score = 20
            else:
                recovery_score = 50

        if injury:
            status = (injury["status"] or "").lower()
            pain = injury["pain"] or 0
            ...
            if status in ["injured", "recovering"]:
                recovery_score = max(10, recovery_score - 30)
            if pain >= 7:
                recovery_score = max(10, recovery_score - 20)
            elif pain >= 4:
                recovery_score = max(10, recovery_score - 10)

        # ======================================================
        # INJURY RISK (based on injury data)
        # ======================================================
        if injury:
            status = (injury["status"] or "").lower()
            pain = injury["pain"] or 0
            stage = (injury["stage"] or "").lower()
            if isinstance(pain, str):
                try:
                    pain = int(pain)
                except:
            ...
            # Adjust by pain level
            if pain >= 8:
                injury_risk = min(100, injury_risk + 15)
            elif pain >= 5:
                injury_risk = min(100, injury_risk + 10)

            # Adjust by stage
            if stage == "acute":
                injury_risk = min(100, injury_risk + 10)
            elif stage == "chronic":
                injury_risk = min(100, injury_risk + 5)
        else:
            injury_risk = 5

        # ======================================================
        # CONFIDENCE SCORE (based on training consistency and injury)
        # ======================================================
        confidence_score = consistency_score
        if injury:
            status = (injury["status"] or "").lower()
            if status == "injured":
                confidence_score = max(20, confidence_score - 25)
            elif status == "recovering":
                confidence_score = max(30, confidence_score - 15)

        if tournament:
            importance = (tournament["importance"] or "").lower()
            ...
            if days_left is not None:
                if isinstance(days_left, str):
                    try:
                        ...
                        days_left = 30
                if days_left <= 7 and importance in ["high", "critical"]:
                    confidence_score = min(100, confidence_score + 10)

        # ======================================================
        # PERFORMANCE SCORE (composite)
        # ======================================================
        performance_score = (recovery_score + confidence_score + consistency_score) // 3

        # Cap performance if injury_risk is high
        if injury_risk > 75:
            performance_score = min(60, performance_score)

        # ======================================================
        # BURNOUT RISK (based on fatigue and training load)
        # ======================================================
        if training:
            fatigue = (training["fatigue"] or "").lower()
            days = training["days"] or 0
            ...
            if fatigue == "very high" or (days >= 6 and minutes >= 90):
                burnout_risk = "high"
            elif fatigue == "high" or (days >= 5 and minutes >= 60):
                burnout_risk = "medium"
            else:
                burnout_risk = "low"
        else:
        ...
        # ======================================================
        if tournament:
            upcoming = (tournament["upcoming"] or "").lower()
            days_left = tournament["days_left"]
            importance = (tournament["importance"] or "").lower()

            if upcoming in ["no", "none", ""]:
                tournament_readiness = 50
            else:
                if days_left is not None:
                    if isinstance(days_left, str):
                    ...
                    # Base readiness from consistency
                    tournament_readiness = consistency_score

                    # Adjust by days left
                    if days_left <= 3:
                        tournament_readiness = min(100, tournament_readiness + 20)
                    elif days_left <= 7:
                        tournament_readiness = min(100, tournament_readiness + 10)
                    elif days_left <= 14:
                        tournament_readiness = min(100, tournament_readiness + 5)

                    # Penalize if injured
                    if injury_risk > 50:
                        tournament_readiness = max(20, tournament_readiness - 20)

                    # Boost if recovery is good
        ...
        injury_risk = max(0, min(100, injury_risk))
        performance_score = max(0, min(100, performance_score))
        tournament_readiness = max(0, min(100, tournament_readiness))
        ...
        # ======================================================
        conn.execute("""
            INSERT OR REPLACE INTO athlete_metrics
            (user_id, recovery_score, confidence_score, injury_risk, performance_score,
             burnout_risk, tournament_readiness, consistency_score, last_calculated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (user_id, recovery_score, confidence_score, injury_risk, performance_score,
              burnout_risk, tournament_readiness, consistency_score, datetime.now().isoformat()))
        conn.commit()

        # ======================================================
        # IMPROVEMENT 2: WEEKLY SNAPSHOT AUTO-CREATION
        # ======================================================
        ...
        # Check if snapshot for this user + week exists
        existing_snapshot = conn.execute(
            "SELECT 1 FROM weekly_snapshots WHERE user_id=? AND week_start=?",
            (user_id, week_start)
        ).fetchone()

        # If not exists, insert a new snapshot
        if not existing_snapshot:
            conn.execute("""
                INSERT INTO weekly_snapshots 
                (user_id, week_start, recovery_score, confidence_score, injury_risk, 
                 performance_score, burnout_risk, tournament_readiness, consistency_score, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (user_id, week_start, recovery_score, confidence_score, injury_risk,
                  performance_score, burnout_risk, tournament_readiness, consistency_score,
                  datetime.now().isoformat()))
            conn.commit()

        conn.close()
    except Exception as e:
        print(f"Metrics recalculation error: {e}")
        pass
...
def signup():
    ...
    try:
        ...
        initialize_metrics_if_missing(new_user_id)
...
def login():
    ...
    try:
        ...
        initialize_metrics_if_missing(user["id"])
...
def save_training():
    ...
    try:
        ...
        log_event(uid, "training_saved")
        recalculate_metrics(uid)
...
def save_tournament():
    ...
    try:
        ...
        log_event(uid, "tournament_saved")
        recalculate_metrics(uid)
        return jsonify(success=True)
    except Exception as e:
...
def dashboard():
    ...
    # ======================================================
    # SERVER-ENFORCED SETUP GATING - Direct database check
    # ======================================================
    try:
        conn = get_db()
        training_exists = conn.execute(
            "SELECT 1 FROM training WHERE user_id=?", (user_id,)
        ).fetchone()
        diet_exists = conn.execute(
            "SELECT 1 FROM diet WHERE user_id=?", (user_id,)
        ).fetchone()
        conn.close()
        
        # If either is missing: redirect to /setup
        if not training_exists or not diet_exists:
            return redirect(url_for("setup"))
    except Exception:
        # On error, allow access (fail-open for UX)
        pass
    
    # IMPROVEMENT 3: Fetch athlete_metrics for dashboard
    metrics = None
    try:
        conn = get_db()
        metrics_row = conn.execute(
            "SELECT * FROM athlete_metrics WHERE user_id=?", 
            (user_id,)
        ).fetchone()
        conn.close()
        
        # If no row found, initialize and fetch again
        if not metrics_row:
            initialize_metrics_if_missing(user_id)
            conn = get_db()
            metrics_row = conn.execute(
                "SELECT * FROM athlete_metrics WHERE user_id=?", 
                (user_id,)
        ...
        # Convert to dictionary if row exists
        if metrics_row:
            metrics = {
                "recovery_score": metrics_row["recovery_score"],
                "confidence_score": metrics_row["confidence_score"],
                "injury_risk": metrics_row["injury_risk"],
                "performance_score": metrics_row["performance_score"],
                "burnout_risk": metrics_row["burnout_risk"],
                "tournament_readiness": metrics_row["tournament_readiness"],
                "consistency_score": metrics_row["consistency_score"],
                "last_calculated_at": metrics_row["last_calculated_at"]
            }
    except Exception:
        # Fallback to default metrics if any error
        metrics = {
            "recovery_score": 75,
            "confidence_score": 50,
            "injury_risk": 10,
    ...
    try:
        conn = get_db()
        # Fetch last 2 weekly_snapshots ordered by week_start DESC
        snapshots = conn.execute(
            "SELECT performance_score FROM weekly_snapshots WHERE user_id=? ORDER BY week_start DESC LIMIT 2",
            (user_id,)
        ).fetchall()
        conn.close()
        
        # If at least 2 snapshots exist, compare performance_score
        if len(snapshots) >= 2:
            current_perf = snapshots[0]["performance_score"]
            previous_perf = snapshots[1]["performance_score"]
            if current_perf > previous_perf:
                trend = "up"
            elif current_perf < previous_perf:
                trend = "down"
            else:
                trend = "stable"
        else:
    ...
    # ======================================================
    # PERFORMANCE HISTORY FOR SVG GRAPH
    # ======================================================
    performance_history = []
    try:
        conn = get_db()
        # Fetch last 4 weekly_snapshots ordered by week_start DESC
        history_snapshots = conn.execute(
            "SELECT performance_score FROM weekly_snapshots WHERE user_id=? ORDER BY week_start DESC LIMIT 4",
            (user_id,)
        ).fetchall()
        conn.close()
        
        # Extract performance_score into a list
        performance_history = [row["performance_score"] for row in history_snapshots]
        
        # Reverse list so oldest first
        performance_history.reverse()
        
        # If fewer than 4 exist, pad with current performance_score
        current_perf_score = metrics.get("performance_score", 60) if metrics else 60
        while len(performance_history) < 4:
            performance_history.insert(0, current_perf_score)
    except Exception:
        # Fallback: use current performance_score for all 4 points
        current_perf_score = metrics.get("performance_score", 60) if metrics else 60
        performance_history = [current_perf_score, current_perf_score, current_perf_score, current_perf_score]
    
    # ======================================================
    # MOMENT REPLAY CALCULATION
    # ======================================================
    moment_replay = None
    try:
        if len(performance_history) >= 2:
            ...
            change = latest - first
            if change != 0:
                percent_change = int((change / max(first, 1)) * 100)
                moment_replay = percent_change
    except Exception:
        moment_replay = None
    
    # ======================================================
    # TOURNAMENT PROJECTION (7-day rule-based projection)
    # ======================================================
    projection = metrics["performance_score"] if metrics else 60
    if trend == "up":
        projection = min(100, projection + 5)
    elif trend == "down":
        projection = max(0, projection - 3)
    else:
        projection = min(100, projection + 2)
    
    return render_template("dashboard.html", user_role=role, metrics=metrics, trend_direction=trend, performance_history=performance_history, moment_replay=moment_replay, projection=projection)
...
def coach_settings_api():
    ...
    try:
        ...
        # If no row exists, auto-insert default row
        if not row:
            conn.execute("""
                INSERT INTO coach_settings (user_id, ai_enabled, style, reply_length)
                VALUES (?, 0, 'calm', 'short')
            """, (uid,))
            conn.commit()
            conn.close()
        ...
        print("Coach settings API error:", e)

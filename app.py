...
def get_memory_anchor(context, mode):
    ...
    if mode == "diet" and context.get("diet"):
        d = context["diet"]
        # Shorter memory anchor for diet mode
        if d.get("allergies") and d["allergies"].strip():
            return f"Noted: {d['allergies']} restrictions."
...
def get_tone_instructions(tone, mode):
    ...
    mode_adaptations = {
        ...
        "diet": {
            "calm": "Make nutrition approachable. Keep it simple and practical.",
            "strict": "Be clear and direct about nutritional needs. No unnecessary details.",
            "motivational": "Frame nutrition as fuel for performance. Keep energy high.",
            "analytical": "Focus on key macros and timing. Be concise and specific."
        }
    }
...
def build_ai_system_prompt(mode, context, depth="short"):
    ...
    # Depth/clarity instructions - DIET MODE ALWAYS SHORT
    if mode == "diet":
        clarity_instruction = "Keep responses SHORT (2-3 sentences max). Be direct and actionable. No lengthy explanations."
    else:
        depth_instructions = {
    ...
    # WHY-TRUST system (mandatory) - DIET MODE USES COMPACT DO/DON'T FORMAT
    if mode == "diet":
        trust_system = """
RESPONSE FORMAT (MANDATORY FOR DIET):
Keep it short. Every response MUST end with exactly:

DO: [One specific action]
DON'T: [One thing to avoid]

Example:
"Eat protein within 30 min after training for better recovery.

DO: Add chicken or eggs to your post-workout meal.
...
Focus: Give ONE practical nutrition tip. End with DO/DON'T. Keep it brief."""
...
Note: AI offline. Return for detailed periodization."""
        ...
        "diet": {
            "calm": f"""{greeting}
{memory_line}
Balanced meals with protein and veggies support your {sport} performance.

DO: Add protein to every meal.
DON'T: Skip meals before training.""",

            "strict": f"""{greeting}
{memory_line}
Eat clean. Protein, vegetables, whole grains. No shortcuts.

DO: Prep meals in advance.
DON'T: Rely on processed foods.""",

            "motivational": f"""{greeting}
{memory_line}
Fuel like the athlete you are! Good nutrition powers great performance.

DO: Eat protein within 30 min post-workout.
DON'T: Skip breakfast on training days.""",

            "analytical": f"""{greeting}
{memory_line}
Target 1.6-2g protein/kg bodyweight. Time carbs around training.

DO: Track protein intake this week.
DON'T: Train fasted for high-intensity sessions."""
        },
...
Note: AI offline. Follow taper protocol."""
    ...
    # Adjust length for depth (but diet is always short)
    if mode != "diet" and depth == "short" and len(response) > 600:
...
def coach():
    ...
    - WHY-TRUST: WHAT + WHY + RISK format (DO/DON'T for diet)
    ...
    try:
        ...
        # Determine reply depth (smart clarity) - diet always short
        if mode == "diet":
            depth = "short"
        else:
...
RISK: Generic advice may not optimize your {user_sport} performance."""
        ...
        if client is None:
            ...
            try:
                ...
                # Diet mode uses fewer tokens for shorter responses
                if mode == "diet":
                    max_tokens = 150
                else:

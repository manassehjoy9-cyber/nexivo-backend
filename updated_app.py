...
# ======================================================
# SPORT-SPECIFIC TOURNAMENT CATEGORIES
# ======================================================
SPORT_TOURNAMENT_OPTIONS = {
    "badminton": ["Singles", "Doubles", "Mixed Doubles"],
    "tennis": ["Singles", "Doubles", "Mixed Doubles"],
    "cricket": ["Practice Match", "League Match", "Tournament Match"],
    "football": ["Friendly", "League", "Knockout"],
    "soccer": ["Friendly", "League", "Knockout"],
    "basketball": ["Friendly", "League", "Knockout"],
    "volleyball": ["Friendly", "League", "Knockout"],
    "running": ["5K", "10K", "Half Marathon", "Full Marathon"],
    "cycling": ["Time Trial", "Road Race", "Criterium"],
    "swimming": ["Sprint", "Distance", "Relay"],
    "fitness": ["Challenge", "Program", "Competition"],
    "gym": ["Challenge", "Program", "Competition"],
    "martial arts": ["Sparring", "Competition", "Grading"],
    "boxing": ["Sparring", "Competition", "Exhibition"],
    "mma": ["Sparring", "Competition", "Exhibition"],
    "wrestling": ["Practice", "Tournament", "Championship"],
    "table tennis": ["Singles", "Doubles", "Mixed Doubles"],
    "squash": ["Singles", "Doubles", "Tournament"],
    "golf": ["Practice Round", "Tournament", "Championship"],
    "hockey": ["Friendly", "League", "Knockout"],
    "rugby": ["Friendly", "League", "Knockout"],
    "default": ["Practice", "Competition", "Championship"]
}

def get_tournament_options_for_sport(sport):
    """Get sport-specific tournament category options"""
    if not sport:
        return SPORT_TOURNAMENT_OPTIONS["default"]
    sport_lower = sport.lower().strip()
    for key in SPORT_TOURNAMENT_OPTIONS:
        if key in sport_lower or sport_lower in key:
            return SPORT_TOURNAMENT_OPTIONS[key]
    return SPORT_TOURNAMENT_OPTIONS["default"]
...
def init_db():
    ...
    # Add advanced_schedule column for academy/elite training
    safe_add_column(cur, "training", "advanced_schedule", "TEXT")
...
def get_user_context(user_id):
    ...
    try:
        ...
        training = conn.execute(
            "SELECT days, minutes, fatigue, plan_start_date, advanced_schedule FROM training WHERE user_id=?",
            (user_id,)
        ).fetchone()
        if training:
            context["training"] = {
                ...
                "advanced_schedule": training["advanced_schedule"] if "advanced_schedule" in training.keys() else None
            }
        ...
        # Memory anchor: get last coach message for context
        last_msg = conn.execute(
            "SELECT message, mode FROM coach_messages WHERE user_id=? AND role='coach' ORDER BY id DESC LIMIT 1",
            (user_id,)
        ).fetchone()
        if last_msg:
...
def get_memory_anchor(context, mode):
    ...
    if mode == "training" and context.get("training"):
        tr = context["training"]
        # Check for advanced/academy schedule first
        if tr.get("advanced_schedule"):
            return f"Your training schedule: {tr['advanced_schedule'][:100]}..."
...
"""
    ...
    if mode == "training" and context.get("training"):
        tr = context["training"]
        # Check for advanced/academy training
        if tr.get("advanced_schedule"):
            mode_context = f"""
TRAINING DATA (ACADEMY/ELITE):
- Schedule: {tr.get('advanced_schedule', 'custom schedule')}
- Days per week: {tr.get('days', '?')}
- Fatigue: {tr.get('fatigue', 'unknown')}
- Plan started: {tr.get('plan_start_date', 'unknown')}
IMPORTANT: This is an advanced/academy athlete with multiple daily sessions. 
Do NOT flag this as overtraining. Instead, analyze session structure and recovery quality.
Focus: session balance, periodization, recovery windows between sessions for {sport}."""
        else:
            mode_context = f"""
TRAINING DATA:
...
def save_training():
    ...
    try:
        ...
        advanced_schedule = data.get("advanced_schedule", "")

        # For advanced/academy training, minutes can be 0 or "custom"
        if advanced_schedule and advanced_schedule.strip():
            # Academy mode: minutes field is optional, use placeholder
            if minutes is None or minutes == "custom":
                minutes = 0
        else:
            # Standard mode: minutes is required
            if days is None or minutes is None or not fatigue:
                return jsonify(success=False, error="Missing required fields."), 400

        if days is None or not fatigue:
            return jsonify(success=False, error="Missing required fields."), 400

        conn = get_db()
        conn.execute("""
            INSERT OR REPLACE INTO training (user_id, days, minutes, fatigue, plan_start_date, advanced_schedule) 
            VALUES (?, ?, ?, ?, ?, ?)
        """, (uid, days, minutes, fatigue, datetime.now().date().isoformat(), advanced_schedule))
        conn.commit()
        conn.close()
...
def training_options():
    """Return training duration options including advanced/academy option"""
    uid = session.get("user_id")
    if not uid:
        return jsonify({}), 401
    ...
    options = {
        "duration_options": [
            ...
            {"value": "custom", "label": "Advanced / Academy training (custom)"}
        ],
        "days_options": [1, 2, 3, 4, 5, 6, 7],
        "fatigue_options": ["Low", "Moderate", "High", "Very High"]
    }
...
@app.route("/api/tournament-options")
def tournament_options():
    """Return sport-specific tournament category options"""
    uid = session.get("user_id")
    if not uid:
        return jsonify({}), 401
    ...
    try:
        conn = get_db()
        user = conn.execute("SELECT active_sport FROM users WHERE id=?", (uid,)).fetchone()
        conn.close()

        sport = user["active_sport"] if user else None
        categories = get_tournament_options_for_sport(sport)

        return jsonify({
            "sport": sport,
            "category_options": categories,
            "importance_options": ["Low", "Medium", "High", "Critical"]
        })
    except Exception:
        return jsonify({
            "category_options": SPORT_TOURNAMENT_OPTIONS["default"],
            "importance_options": ["Low", "Medium", "High", "Critical"]
        })
